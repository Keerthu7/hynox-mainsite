...
export default function PricingPage() {
  ...
  function getWhatsAppLink(pkg: { name: string }, type: "development" | "management" | "marketing") {
    // Find corresponding packages by name
    const dev = developmentPackages.find(p => p.name === pkg.name);
    const mgmt = managementPackages.find(p => p.name === pkg.name);
    const mktg = marketingPackages.find(p => p.name === pkg.name);

    // Use deterministic price formatting for the development package
    const currencySymbol = isInternationalPricing ? "$" : "₹";
    const formattedPrice = dev
      ? `${currencySymbol}${dev.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}/month`
      : "Contact for Price";

    // Build the WhatsApp message
    const devSection = dev ? `\n\n--- Development ---\n${dev.features.map(f => `• ${f}`).join("\n")}` : "";
    const mgmtSection = mgmt ? `\n\n--- Manufacturing ---\n${mgmt.features.map(f => `• ${f}`).join("\n")}` : "";
    const mktgSection = mktg ? `\n\n--- Marketing ---\n${mktg.features.map(f => `• ${f}`).join("\n")}` : "";

    const message = `Hello, I'm interested in your ${pkg.name} Plan.${devSection}${mgmtSection}${mktgSection}\n\nTotal Price: ${formattedPrice}\n\nCan you please provide me with more information?`;
                  ...
                  <tbody>
                    {[
                      ...
                      },
                      {
                        ...
                        values: ["Email", "12/5", "24/7"]
                      },
                      {
                        feature: "Domain Included",
                        values: ["₹600", "₹1500", "₹3000"]
                      },
                      {
                  ...
                  <tbody>
                    {[
                      ...
                      },
                      {
                        ...
                        values: ["Weekly", "Daily", "Continuous"]
                      },
                      {
                        feature: "Included Support",
                        values: ["5h/month", "5h/month", "10h/month"]
                      },
                      {
                        feature: "Development Hours",
                        values: ["None", "5h/month", "10h/month"]
                      },
                      {
                        feature: "Reporting",
